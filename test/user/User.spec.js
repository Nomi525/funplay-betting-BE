...
let userDetails = {
    ...
    wallet: ["JHJjhfsjkdfhskjfskdfjsdfsdk"],
...
describe('User test case', () => {
    ...
    //         .send({ email: userDetails.email, password: userDetails.password, currency: userDetails.currency, type: "signup" })
    //         .end((err, res) => {
    //             if (res.body.status == 200) {
    //                 userId = res.body.data._id;
    //                 res.body.should.have.status(200);
    ...
    //         .send({ wallet: userDetails.wallet })
    //         .end((err, res) => {
    //             console.log(res.body);
    //             if (res.body.status == 200) {
    //                 userId = res.body.data._id;
